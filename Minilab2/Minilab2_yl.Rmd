...
d.parameter_summary =
  d %>%
  group_by(Subject, Sex, Rmax, Slope, C50, Asymp, LogAsymp, LogC50, Contrast, VideoGaming) %>%
  summarise()
...
rmax_t_model = lm(data = d.parameter_summary, Rmax~as.factor(Sex))
tidy(rmax_t_model)
slope_t_model = lm(data = d.parameter_summary, Slope~as.factor(Sex)) 
tidy(slope_t_model)
C50_t_model = lm(data = d.parameter_summary, C50~as.factor(Sex)) 
tidy(C50_t_model)
...
d.out_rmax = d.parameter_summary %>%
  ...
  t.test(data = d.out_rmax, Rmax~as.factor(Sex))
...
# 
# t.test(data = d.out_slope, Slope~as.factor(Sex))  
# 
# # C50 outliers
# d.out_c50 = d.parameter_summary %>%
#   filter(C50 < 3*sd(C50))
# 
# t.test(data = d.out_c50, C50~as.factor(Sex)) 
...
anova(rmax_t_model, slope_t_model, test = "Chisq")
anova(rmax_t_model, C50_t_model, test = "Chisq")
anova(slope_t_model, C50_t_model, test = "Chisq")
...
# histogram plot for video game by gender
ggplot(d.parameter_summary) + geom_histogram(aes(x = Rmax, fill= as.factor(VideoGaming)), alpha = 0.8) +
  geom_density(aes(x = Rmax, color = Sex), alpha = 0.2) +
  theme_classic() + labs(title = "Histogram distribution of Rmax and Video Gaming") + 
  theme(plot.title = element_text(hjust=0.5, size = 20))
