...
d.parameter_summary =
  d %>%
  group_by(Subject, Sex, Rmax, Slope, C50, Asymp, LogAsymp, LogC50) %>%
  summarise()


# lm and t-tests 
tidy(glm(data = d.contrast_summary, meanLogDuration~Contrast + as.factor(Sex)))
t.test(data = d.parameter_summary, Rmax~as.factor(Sex))
t.test(data = d.parameter_summary, Slope~as.factor(Sex))
t.test(data = d.parameter_summary, C50~as.factor(Sex))
```
...
# t-tests
t.test(data = d.parameter_summary, Asymp~as.factor(Sex))
t.test(data = d.parameter_summary, LogAsymp~as.factor(Sex))
t.test(data = d.parameter_summary, LogC50~as.factor(Sex))
  
# Analysis of paramater after filtering out +/- 3SD of mean 

# Rmax outliers
d.out_rmax = d.parameter_summary %>%
  filter(abs(d.parameter_summary$Rmax) < 3*sd(d.parameter_summary$Rmax)+(d.parameter_summary$Rmax)) #error
